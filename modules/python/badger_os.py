...
def get_battery_level():
    return 0
    # Battery measurement
    vbat_adc = machine.ADC(badger2350.PIN_BATTERY)
    vref_adc = machine.ADC(badger2350.PIN_1V2_REF)
    vref_en = machine.Pin(badger2350.PIN_VREF_POWER)
...
def launch(file):
    ...
    def quit_to_launcher(_pin):
        state_clear_running()
        os.sync()
        while badger2350.BUTTON_HOME.value() == 0:
            pass
        machine.reset()

    badger2350.BUTTON_HOME.irq(trigger=machine.Pin.IRQ_FALLING, handler=quit_to_launcher)
...
def warning(display, message, width=badger2350.WIDTH - 20, height=badger2350.HEIGHT - 20, line_spacing=20, text_size=0.6):
    print(message)
    ...
    display.set_pen(12)
    display.rectangle((badger2350.WIDTH - width) // 2, (badger2350.HEIGHT - height) // 2, width, height)
    ...
    for i in range(num_lines):
        ...
        display.text(lines[i], (badger2350.WIDTH - length) // 2, (badger2350.HEIGHT // 2) + current_line, badger2350.WIDTH, text_size)
